...
def handle_send_msg(data):
    global client_socket
    data = eval(data)
    emit("new_msg", {"targetRoom": data["roomName"], "msg": data["msg"]}, broadcast=True)
    client_socket.send(data)
    msg_b = bytes(data["msg"], "utf-8")
    emit(
        "msg_to_byte", {"byte": str(msg_b), "orderedByte": "orderedByte"}, to=data["sender"]
    )
    emit(
        "recv_msg", {"sender": data["sender"], "msg": data["msg"], "targetRoom": data["roomName"]}, to=data["roomName"]
    )
