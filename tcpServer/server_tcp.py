    ...
    def tcp_server(self):
        try:
            ...
            while not self.stop_flag:
                ...
                client = Thread(target=self.client_thread, args=(client_socket, client_addr))
                print(f"{client_addr[0]}:{client_addr[1]} : Connected {len(self.client_list)}/{self.max_connection}")
                client.start()
            self.socket.close()
        except OSError as e:
